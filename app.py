...
from src.doc_classifier_dl import VGG16DocumentClassifier

def predict_doc_type(input_img):

    req_id = str(uuid.uuid4())
    # folder_save_path = f"./src/completed_requests/{req_id}"
    ...
    predictions = vgg_model.predict(img_path=input_img, classes=labels)
...
with gr.Blocks() as demo:
    ...
    ---
    Steps to test model:
    - Upload an image by clicking the document uploader button below, or select one of the examples from the 'Examples' Sections
    - Click on predict
    ...
    with gr.Row():
        form_image = gr.Image(type="pil", shape=(256, 256))
    ...
    gr.Examples(
        # examples=["./src/examples/82_submission_sheet.png", "./src/examples/form_d.png"],
        examples=[os.path.join("./src/examples", img_path) for img_path in os.listdir("./src/examples")],
        inputs=form_image,
        outputs=pred_result,
        fn=predict_doc_type,
        # cache_examples=True
